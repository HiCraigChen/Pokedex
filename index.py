...
import numpy as np
import argparse
import pickle  #no need cPickle or _pickle
import glob
import cv2
...
for spritePath in glob.glob(args["sprites"] + "/*.png"):
	...
	# of the image
	image = cv2.copyMakeBorder(image, 15, 15, 15, 15, cv2.BORDER_CONSTANT, value = 255) #value=255 > white
	...
	imggg, cnts, heir = cv2.findContours(thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

	#cnts, _ = cv2.findContours(thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
	#In OpenCV, finding contours is like finding white object from black background. So remember, object to be found should be white and background should be black.
	#If you use RETR_EXTERNAL, it returns only extreme outer flags. All child contours are left behind.
